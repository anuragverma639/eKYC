...
contract RobustKYC_BankA is robustKYC_Contract_Address {
   
   
   function name_by_BankA(uint256 id,robustKYC_Data_Storage.field_status name){
      
       data.name_verification_by_BankA(id,name);
       
   }
   
   function email_by_BankA(uint256 id,robustKYC_Data_Storage.field_status email){
      data.email_verification_by_BankA(id,email);
       
       
   }
   
   function phone_no_by_BankA(uint256 id,robustKYC_Data_Storage.field_status phone_no){
      data.phone_no_verification_by_BankA(id,phone_no);
       
   }
   
   function address_details_by_BankA(uint256 id,robustKYC_Data_Storage.field_status address_details){
       
       data.address_details_verification_by_BankA(id,address_details);
       
   }
   
   function blood_group_by_BankA(uint256 id, robustKYC_New_field.field_status blood_group){
       
        data2.blood_group_verification_by_BankA(id,blood_group);
   }
   
   function image_by_BankA(uint256 id, robustKYC_Customer_Image.field_status image){
      
         image_data.image_verification_by_BankA(id,image);
   }
   
   function document1_by_BankA(uint id , robustKYC_Customer_documents.field_status document1){
       
       document_data.document1_verification_by_BankA(id , document1);
   }
   
    function document2_by_BankA(uint id , robustKYC_Customer_documents.field_status document2){
       document_data.document2_verification_by_BankA(id , document2);
   }
   
    function document3_by_BankA(uint id , robustKYC_Customer_documents.field_status document3){
       document_data.document3_verification_by_BankA(id , document3);
   }
